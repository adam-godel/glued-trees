...
by taking a smaller cropped Pauli list generated by Pauli decomposition and padding its longest substring
of identical characters with that character for the desired qubit size. This is generally reliable 
due to the similar pattern of all the Hamiltonian matrices we are using. The Pauli list generated is
...
def pauli_str(dim):
    ...
    for i in max_cache:
        ...
        if temp > length:
            idx = len(i[0])-1
        result.append((i[0][:idx]+i[0][idx]*(dim-len(i[0]))+i[0][idx:], i[1]))
    return result
