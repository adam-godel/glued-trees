...
# import networkx as nx`)}
            ...
            <p><Latex>The Pauli list representation of {`$\\mathbf{H}$`} for qubit sizes small enough to calculate (12 qubits and lower) are generated and cropped using the {`<code>generate_pauli_list</code>`} function, which uses an ad hoc approach that tries to represent the Pauli list as a whole as accurately as possible using only the 200 ostensibly most relevant terms to simulating the system.</Latex></p>
            ...
            {codeBlock(`def crop_pauli_list(pauli_list, size):
    ...
    while len(result) < round(size * 0.6):
        for i in range(len(pauli_list[0].pauli) - 1, -1, -1):
            for k in pauli_list:
                if k.pauli[i] == [Pauli.I, Pauli.X, Pauli.Y, Pauli.Z][idx % 4] and k not in result:
                    result.append(k)
                    break
            idx += 1
    for i in pauli_list:
...
def generate_pauli_list(qubits):
    ...
    return crop_pauli_list(sorted(result, key=lambda x: abs(x.coefficient), reverse=True), 200)
...
def pauli_str(qubits, recalculate=False):
    ...
    if qubits > 12:
        return [
            PauliTerm([i.pauli[0]] + [i.pauli[1]] * (qubits - len(i.pauli)) + i.pauli[1:], i.coefficient)
...
def run_range(qubits, recalculate=False):
    ...
    for i in range(-12, 13, 2):
        run_point(qubits, 2*qubits-4+i, pauli_list)`)}
            ...
            <p><Latex>The following code segment, on the other hand, displays the execution of {`<code>run_range</code>`} for 20 qubits ($n=18, N=524286$), a qubit size that is still simulatable but where its Pauli list cannot be generated and is thus approximated using the Pauli list for 12 qubits.</Latex></p>
            <p>This instance of the function should take a few minutes to run if the cached Pauli list is used. Since this instance of the function requires generating the Pauli list for 12 qubits, which is very large, it will take much longer to run if you choose to recalculate the Pauli list.</p>
    ...
    plt.show()`)}
            <p><Latex>The following code segment displays the graph for 10 qubits, a qubit size with a generated and cropped Pauli list. As expected, there is a large spike at $t \approx 2n$ indicative of the initial push to the entrance node having "reached" the exit node.</Latex></p>
            {codeBlock(`graph_results(10)`)}
            <img src={TenQubits}/>
            <p><Latex>The following code segment displays the graph for 20 qubits, a qubit size with an approximated Pauli list. There is a clear spike at $t \approx 2n$ caused by the propagation from the entrance node.</Latex></p>
